...
namespace folio1_app_test.Services
{
    ...
    public class StudentService : IStudentService
    {
        ...
        private void SeedData()
        {
            ...
            {
                dbContext.Students.Add(new StudentDB() { id = 1, folioClassId = 1, fname = "John", lname = "Packer", age = 18, gpa = 3.2 });
                dbContext.Students.Add(new StudentDB() { id = 2, folioClassId = 1, fname = "Peter", lname = "Johnston", age = 19, gpa = 2.5 });
                dbContext.Students.Add(new StudentDB() { id = 3, folioClassId = 1, fname = "Robert", lname = "Smith", age = 20, gpa = 3.1 });
                dbContext.Students.Add(new StudentDB() { id = 4, folioClassId = 1, fname = "Louise", lname = "Thomstone", age = 21, gpa = 2.1 });

                dbContext.Students.Add(new StudentDB() { id = 5, folioClassId = 2, fname = "JohnP", lname = "JPacker", age = 18, gpa = 3.3 });
                dbContext.Students.Add(new StudentDB() { id = 6, folioClassId = 2, fname = "PeterJ", lname = "PJohnston", age = 19, gpa = 2.5 });
                dbContext.Students.Add(new StudentDB() { id = 7, folioClassId = 2, fname = "RobertS", lname = "RSmith", age = 20, gpa = 3.2 });
                dbContext.Students.Add(new StudentDB() { id = 8, folioClassId = 2, fname = "LouiseT", lname = "LThomstone", age = 21, gpa = 3.9 });
        ...
        public async Task<(bool IsSuccess, string Message)> CheckDuplicateAsync(int studentId, string studentLastName)
        {
            try
            {
                ...
                if (studentId > 0)
                {
                    student = await dbContext.Students.Where(x => x.lname == studentLastName && x.id != studentId).FirstOrDefaultAsync();
                }
                else
                {
                    student = await dbContext.Students.Where(x => x.lname == studentLastName).FirstOrDefaultAsync();
                }
